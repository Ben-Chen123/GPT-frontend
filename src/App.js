...
function App() {
  ...
  // --- Generate or retrieve persistent user_id (commented out) ---
  ...
  // ============================================================
  // Effects: Response Times, Engagement, Avatars
  // ============================================================
  useEffect(() => {
  ...
  const toggleTimer = async () => {
    ...
    if (!isRunning && taskInitiationDelay === null) {
      const delaySeconds = Math.floor((Date.now() - sessionStartTime) / 1000);
      setTaskInitiationDelay(delaySeconds);
      await saveMetrics({ task_initiation_delay: delaySeconds });
    }

    // ===== PAUSE =====
    if (isRunning) {
      await saveMetrics({ focus_duration: focusDuration });
    }
  ...
  const sendMessage = async () => {
    ...
    if (progressType === "completion") {
      setSuggestedAction(
        "Great progress! What’s the smallest next step you can take right now?"
      );
    }
    if (progressType === "partial") {
      setSuggestedAction(
        "You're in motion. Want to continue for 5 more focused minutes?"
      );
    }
    if (progressType === "effort") {
      setSuggestedAction(
        "Even trying counts. What’s one tiny step you could attempt next?"
      );
    }
    ...
    try {
      ...
      if (data.farewell) {
        ...
        const experimentMetrics = {
          ...
          completion_time: completionTime,
          focus_duration: focusDuration,
          daily_adherence: updatedAdherence,
        };
  ...
  const ProgressFeedback = () => {
    ...
    return (
      <div className="progress-feedback medium">
        🌱 You're making steady progress in the <strong>{displayStage}</strong>{" "}
        stage.
      </div>
    );
  };
  ...
  return (
    <div className="main-layout">
      ...
      <div className="app-container">
        ...
        <div className="input-box">
          <input
            ...
            placeholder={
              isEnded ? "Conversation has ended." : "Type your message..."
            }
            disabled={isEnded}
          />
      ...
      {/* Pomodoro Timer (only shown in Planning stage) */}
      {stage === "Planning" && (
        <div className="pomodoro-timer enhanced">
          <div className="timer-header">
            <h2>{isRunning ? "🎯 Focus Session" : "🍅 Pomodoro Timer"}</h2>
        ...
        {stage === "Planning" && (
          <div className="planner-section">
            <div className="planner-header">
              ...
              <p>
                Turn your intentions into concrete actions. Your personalized
                plan is generated from our conversation.
              </p>
            </div>
              ...
              {isGeneratingPlan ? (
                ...
                <>
                  ...
                  <div className="confidence-section">
                    <label>
                      How confident are you that you'll follow this plan?
                    </label>
                    <div className="confidence-slider-wrapper">
                      <input
                        ...
                        onChange={(e) =>
                          setPlanConfidence(Number(e.target.value))
                        }
                  ...
                  <div className="planner-tip">
                    💡 Tip: The more specific your trigger and action, the more
